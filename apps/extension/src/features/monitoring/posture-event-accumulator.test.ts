...
describe("PostureEventAccumulator", () => {
  it("batches eight valid posture scores", () => {
  ...
  it("warns once after sustained poor posture", () => {
    ...
    expect(accumulator.add(estimate("leaning"), 6_000).warning).toBe("leaning");
  ...
  it("resets the warning episode after healthy posture", () => {
    ...
    expect(accumulator.add(estimate("too-close"), 3_000).warning).toBe("too-close");
    accumulator.add(estimate("healthy"), 6_000);
    accumulator.add(estimate("too-close"), 7_000);
    expect(accumulator.add(estimate("too-close"), 10_000).warning).toBe("too-close");
  });
});
